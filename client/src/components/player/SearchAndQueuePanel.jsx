...
export function SearchAndQueuePanel({ onAddVideo, onPlayVideo, onRemoveVideo }) {
  ...
  return (
    <div className="bg-surface-container/90 backdrop-blur-xl rounded-2xl md:rounded-3xl border border-outline-variant/60 shadow-card overflow-hidden flex flex-col h-full max-h-full" data-testid="search-and-queue-panel">
      {/* Tabs */}
      <div className="flex border-b border-outline-variant/50 bg-surface-container-lowest/80 shrink-0 px-2">
        <button
          type="button"
          onClick={() => setActiveTab('search')}
          className={`flex-1 py-3 flex items-center justify-center gap-2 text-xs sm:text-sm font-semibold transition-all border-b-2 ${activeTab === 'search' ? 'border-primary text-primary bg-primary/10' : 'border-transparent text-on-surface-variant hover:bg-surface-container-highest hover:text-on-surface'}`}
        ...
        <button
          ...
          className={`flex-1 py-3 flex items-center justify-center gap-2 text-xs sm:text-sm font-semibold transition-all border-b-2 ${activeTab === 'queue' ? 'border-primary text-primary bg-primary/10' : 'border-transparent text-on-surface-variant hover:bg-surface-container-highest hover:text-on-surface'}`}
      ...
      {/* Content Area */}
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-3 sm:p-4 flex flex-col">
        
        {/* Search Tab */}
        {activeTab === 'search' && (
          <div className="flex flex-col gap-3.5 h-full min-h-0 flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 shrink-0">
              ...
              <form onSubmit={handleSearch} className="flex gap-1.5 items-center">
                ...
                <button 
                  type="submit" 
                  disabled={loading} 
                  className="btn btn-primary px-4 h-10 rounded-xl min-w-[54px] text-xs sm:text-sm font-semibold shrink-0 shadow-soft"
              ...
              <form onSubmit={handleAddDirectUrl} className="flex gap-1.5 items-center">
                <div className="relative flex-1 min-w-0">
                  <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-muted text-[18px]">link</span>
                  <input
                    type="text"
                ...
                <button 
                  ...
                  className="btn bg-surface-container-highest hover:bg-primary hover:text-on-primary text-on-surface px-3.5 h-10 rounded-xl text-xs sm:text-sm font-semibold shrink-0 transition-colors border border-outline-variant/60"
            ...
            {error && <div className="text-error text-center bg-error-container py-1.5 px-3 rounded-xl text-xs font-semibold border border-error/30" data-testid="search-error">{error}</div>}
            ...
            <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar flex flex-col gap-2.5 pr-1">
              {results.length === 0 && !loading && !error && (
                 <div className="flex flex-col items-center justify-center py-12 text-on-surface-muted">
                    <div className="w-12 h-12 rounded-2xl bg-surface-container-highest/60 border border-outline-variant/60 flex items-center justify-center text-primary mb-2 shadow-soft">
              ...
              {loading && (
                <div className="flex flex-col gap-2.5" data-testid="search-skeleton">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="flex gap-3 bg-surface-container-highest/40 p-2.5 rounded-2xl border border-outline-variant/60 animate-pulse">
                      <div className="w-[100px] h-[58px] shrink-0 rounded-xl bg-surface-container-highest"></div>
                      ...
                      <div className="flex flex-col gap-1.5 shrink-0 justify-center pr-1">
                        <div className="w-8 h-8 rounded-xl bg-surface-container-highest"></div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {results.map((v) => (
                <div key={v.youtubeId} className="flex gap-3 bg-surface-container-highest/40 hover:bg-surface-container-highest/80 p-2 sm:p-2.5 rounded-2xl border border-outline-variant/60 hover:border-primary/40 hover:shadow-card hover:-translate-y-0.5 transition-all duration-200 group" data-testid={`video-result-${v.youtubeId}`}>
                  <div className="w-[96px] sm:w-[110px] h-[56px] sm:h-[62px] shrink-0 rounded-xl overflow-hidden relative shadow-inner bg-black">
                     <img 
                  ...
                  <div className="flex sm:flex-row flex-col gap-1.5 shrink-0 justify-center items-center pr-1">
                    <button 
                      ...
                      disabled={!canControl}
                      className={`flex items-center justify-center gap-1 px-2.5 sm:px-3 h-8 sm:h-9 rounded-xl text-xs sm:text-sm transition-all shadow-soft active:scale-95 ${!canControl ? 'bg-surface-container-highest text-on-surface-muted cursor-not-allowed opacity-50' : 'bg-primary text-on-primary hover:bg-primary-hover'}`}
                    ...
                    <button 
                      ...
                      disabled={!canControl}
                      className={`flex items-center justify-center gap-1 px-2.5 sm:px-3 h-8 sm:h-9 rounded-xl text-xs sm:text-sm transition-all shadow-soft active:scale-95 ${!canControl ? 'bg-surface-container-highest text-on-surface-muted cursor-not-allowed opacity-50' : 'bg-surface-container-highest hover:bg-primary-container text-primary hover:text-on-primary-container border border-outline-variant/60'}`}
        ...
        {activeTab === 'queue' && (
          <div className="flex flex-col gap-3 h-full overflow-y-auto" data-testid="queue-list-container">
            {queue.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full min-h-[200px] text-on-surface-muted" data-testid="queue-empty-state">
                <div className="w-12 h-12 rounded-2xl bg-surface-container-highest/60 border border-outline-variant/60 flex items-center justify-center text-primary mb-2 shadow-soft">
                  <span className="material-symbols-outlined text-3xl">queue_music</span>
                </div>
                <span className="font-semibold text-xs sm:text-sm text-on-surface">Queue is empty</span>
                <span className="text-[11px] text-on-surface-muted mt-0.5">Search for videos or paste a link to add them here</span>
              </div>
            ) : (
              queue.map((v, index) => (
                <div key={v.id || index} className="flex gap-3 bg-surface-container-highest/40 hover:bg-surface-container-highest/80 p-2 rounded-2xl border border-outline-variant/60 relative hover:border-primary/40 hover:shadow-card hover:-translate-y-0.5 transition-all duration-200 group" data-testid={`queue-item-${index}`}>
                  <div className="absolute -top-1.5 -left-1.5 bg-primary text-on-primary text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center shadow-md">
                    {index + 1}
                  </div>
                  <div className="w-[100px] h-[56px] shrink-0 rounded-xl overflow-hidden ml-2 bg-black flex items-center justify-center">
                     {v.thumbnail ? (
                       <img 
                          src={v.thumbnail} 
                          alt={v.title} 
                       ...
                       <span className="material-symbols-outlined text-2xl text-on-surface-muted opacity-50">movie</span>
                     )}
                  </div>
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    <div className="text-xs sm:text-sm font-semibold text-on-surface truncate group-hover:text-primary transition-colors">{v.title}</div>
                    <div className="text-[10px] sm:text-[11px] text-on-surface-muted truncate mt-0.5 flex items-center gap-1">
                  ...
                  {isHost && (
                    <div className="flex flex-col gap-1 shrink-0 justify-center pr-1">
                      <button type="button" onClick={() => handlePlayNow(v)} className="w-7 h-7 rounded-lg bg-primary text-on-primary hover:bg-primary-hover flex items-center justify-center transition-colors shadow-soft" title="Play Now" data-testid={`queue-play-${index}`}>
                        <span className="material-symbols-outlined text-[16px] fill-1">play_arrow</span>
                      </button>
                      <button type="button" onClick={() => handleRemoveFromQueue(v.id)} className="w-7 h-7 rounded-lg bg-error-container text-error hover:bg-error hover:text-on-error flex items-center justify-center transition-colors" title="Remove" data-testid={`queue-remove-${index}`}>
